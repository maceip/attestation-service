//! Platform-specific TEE quote verification. VENDORED from
//! `maceip/unified-quote` (`v2/src/quote/verify.rs`). Only change from
//! upstream: the AMD KDS module path (`crate::stackcore::kds`).
//!
...
fn verify_snp_quote(
    ...
    let mut chain_verified = false;
    {
        ...
        if vcek_der.is_none() {
            if let Ok((product, chip_id, bl, tee, snp_ver, ucode)) =
                crate::stackcore::kds::extract_kds_params(raw_quote)
            {
                match crate::stackcore::kds::fetch_vcek(&product, &chip_id, bl, tee, snp_ver, ucode)
                {
                    Ok(vcek) => {
                        vcek_der = Some(vcek);
                        if let Ok((ask, ark)) = crate::stackcore::kds::fetch_cert_chain(&product) {
                            ask_der = Some(ask);
                            ark_der = Some(ark);
                        }
                    }
                    Err(e) => {
                        eprintln!("[attestation-service/verify] AMD KDS fetch failed: {e}");
                    }
                }
            }
        }
        ...
        if let Some(ref vcek) = vcek_der {
            ...
            if let (Some(ref ask), Some(ref ark)) = (&ask_der, &ark_der) {
                verify_cert_chain_p384(ark, ark)?;
                let version =
                ...
                verify_cert_chain_p384(ark, ask)?;
                verify_cert_chain_p384(ask, vcek)?;
                chain_verified = true;
            }
        }
    }
    ...
    let mut measurements = vec![
        ...
        ("SIG_VERIFIED".to_string(), vec![if sig_verified { 1 } else { 0 }]),
        (
...
fn verify_tdx_quote(
                    ...
                    if der_certs.len() >= 2 {
                        ...
                        if der_certs.len() >= 3 {
                            ...
                            if chain_ok {
                                ...
                                if !chain_ok {
                                    eprintln!("[attestation-service/verify] TDX: Intel root CA fingerprint mismatch");
                                }
                            }
    ...
    let measurements = vec![
        ...
        ("QE_SIG_VERIFIED".to_string(), vec![if qe_verified { 1 } else { 0 }]),
        (
            "CHAIN_VERIFIED".to_string(),
            vec![if chain_verified { 1 } else { 0 }],
        ),
